...
/// <summary>
/// Дверь внутри Лабиринта, через которую Аида вошла. Если выходим через нее, попадаем в земли Аиды.
/// </summary>
public class LabyrinthEntrance : MonoBehaviour
{
	[Tooltip ("Название сцены, представляющей земли Аиды.")]
	public string aidaLandSceneName;
	[Tooltip ("Название объекта, представляющего Start Point рядом со входом в Лабиринт.")]
	public string labyrinthStartPointName;
	...
	{
		// При выходе через дверь персонаж появляется рядом со входом в Лабиринт.
		GlobalManager.instance.playerStartPointName = labyrinthStartPointName;
		StartCoroutine (LoadNextScene ());
	}

	private IEnumerator LoadNextScene ()
	{
		AsyncOperation asyncLoad = SceneManager.LoadSceneAsync (aidaLandSceneName);
